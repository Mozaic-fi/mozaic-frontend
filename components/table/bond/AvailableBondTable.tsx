...
const AvailableBond = ({ items, openBond }) => {
        ...
        <table>
            ...
            <thead>
                <tr>
                    ...
                    <th>
                        <div>
                            <p className="table-header table-title tc-s df-c">{width > 530 ? "ROI" : "ROI & TVP"}</p>
                        </div>
                    </th>
                    {width > 530 && <th>
                        <div>
                    ...
                    <tr key={item.id}>
                            ...
                            ? <>
                                <td>
                                    <div className="table-body df-c">
                                        <p style={item.roi < 0 ? { color: "#EE3458", margin: 0 } : { margin: 0 }}>{item.roi}%</p>
                                    </div>
                                </td>
                                <td>
                                    <div className="table-body df ai-c jc-c">
                                        <div className="df-ca df fd-c ai-c jc-c mr-0">
                                            ...
                                            {width > 530 && <ProgressBar maxCap={100} currDep={50} />}
                                        </div>
                                    </div>
                                </td>
                            </>
                            : <td>
                                <div className="table-body">
                                    <div style={{ marginRight: 0 + "!important" }}>
                                        <p className="fs-xs mr-0">ROI: <span className="t-b tc-p fs-xs">{item.roi}%</span></p>
                                        <p className="fs-xs mr-0">TBV: <span className="t-b tc-p fs-xs">{item.tbv}</span></p>
                                    </div>
                                </div>
                            </td>
                        }
                        <td>
                            <div className="table-body  tc-s df ai-c jc-e">
                                {width > 560
                                    ? <button onClick={() => { openBond(item.id) }} className="btn-primary">Bond</button>
                                    : <button onClick={() => { openBond(item.id) }} className="btn-primary pl-1 pr-1 df-c"><img className="mr-0" src="/assets/icons/ico.bondbadge.svg" alt="" /></button>
                                }
                            </div>
                        </td>
                    </tr>
